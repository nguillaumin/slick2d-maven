...
public class Cursor {
     ...
     */
    public Cursor(int width, int height, int xHotspot, int yHotspot, int numImages, IntBuffer images, IntBuffer delays) throws SlickException {
        synchronized (GlfwPackageAccess.global_lock) {
            if ((getCapabilities() & CURSOR_ONE_BIT_TRANSPARENCY) == 0)
                throw new SlickException("Native cursors not supported");
            // TODO why was bufferChecks deprecated? maybe this already happens?
...
//                BufferChecks.checkBufferSize(delays, numImages);
            ...
            // FIXME probably need to bind glfw listeners here
//            Sys.initialize();
    ...
    public static int getCapabilities() {
        synchronized (GlfwPackageAccess.global_lock) {
            if (Mouse.getImplementation() != null)
                ...
                // FIXME this is wrong
                return 0;
//                return GlfwPackageAccess.createImplementation().getNativeCursorCapabilities();
        }
    }
